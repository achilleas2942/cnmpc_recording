        ...
        for topic, msg, t in bag.read_messages(topics=[topic, agents_topic]):
            ...
            if data_agents < 70:
                ...
                for i in range(0, int(len(data)/4)):
                    ...
                    if k8s_node_init == 1:
                        temp_node_1.append((num_agents_init, horizon_init, resource_init))
                    elif k8s_node_init == 2:
                        temp_node_2.append((num_agents_init, horizon_init, resource_init))
                    elif k8s_node_init == 3:
                        temp_node_3.append((num_agents_init, horizon_init, resource_init))

                # Append temporary lists to the main lists, using empty lists for missing data
                num_agents_node_1.append([val[0] for val in temp_node_1] if temp_node_1 else [0])
                horizon_node_1.append([val[1] for val in temp_node_1] if temp_node_1 else [0])
                resources_node_1.append([val[2] for val in temp_node_1] if temp_node_1 else [0])

                num_agents_node_2.append([val[0] for val in temp_node_2] if temp_node_2 else [0])
                horizon_node_2.append([val[1] for val in temp_node_2] if temp_node_2 else [0])
                resources_node_2.append([val[2] for val in temp_node_2] if temp_node_2 else [0])

                num_agents_node_3.append([val[0] for val in temp_node_3] if temp_node_3 else [0])
                horizon_node_3.append([val[1] for val in temp_node_3] if temp_node_3 else [0])
                resources_node_3.append([val[2] for val in temp_node_3] if temp_node_3 else [0])

    return (num_agents_node_1, horizon_node_1, resources_node_1,
            num_agents_node_2, horizon_node_2, resources_node_2,
            num_agents_node_3, horizon_node_3, resources_node_3)

# Function to plot lists of lists
def plot_data_with_markers(axis, data, title, ylabel, color, alpha=0.4):
    markers = ['o', '+', 'x', 'o', '+', 'x', 'o', '+', 'x', 'o', '+', 'x']  # List of marker styles
...
num_agents_node_1, horizon_node_1, resources_node_1, \
num_agents_node_2, horizon_node_2, resources_node_2, \
num_agents_node_3, horizon_node_3, resources_node_3 = extract_rosbag_data(bag_file)
...
with plt.style.context(["science", "ieee"]):
    ...
    plot_data_with_markers(axes[0, 0], num_agents_node_1, '', r'(a) $n_\text{apc,j}(t)$', 'blue')
    ...
    plot_data_with_markers(axes[1, 0], horizon_node_1, '', r'(b) $h_\text{j}(t)$', 'blue')
    ...
    plot_data_with_markers(axes[2, 0], resources_node_1, '', r'(c) $r_\text{opt,j}(t)$', 'blue')
